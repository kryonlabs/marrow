 ...
 * /dev/audio - Audio output via ALSA (9front compatible)
...
/*
 * Initialize ALSA device (stereo by default for 9front compatibility)
 */
static int init_alsa(int channels)
{
#ifdef USE_ALSA
    int err;
    int dir = 0;
...
#else
    (void)channels;
    fprintf(stderr, "devaudio: ALSA not available - audio disabled\n");
    return -1;
#endif
}
